<template>
  <div>
    ...
    >
      <div v-show="isOpen" class="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-40 transition-opacity" @click="$emit('close')"></div>
    </transition>
    ...
    >
      <div v-show="isOpen" class="fixed right-0 top-0 h-full w-full w-full md:w-[480px] lg:w-[600px] bg-slate-900 border-l border-slate-700 z-50 shadow-2xl overflow-y-auto flex flex-col">
        ...
        <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between sticky top-0 bg-slate-900/90 backdrop-blur z-10">
          ...
          <button @click="$emit('close')" class="text-slate-400 hover:text-white transition p-2 hover:bg-slate-800 rounded-full">
        ...
        <div class="p-6 flex-1 flex flex-col gap-6">
          <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-2">
              <label class="text-sm font-semibold text-slate-400 uppercase tracking-wider">目标资源 (URL)</label>
              <div class="flex gap-2">
                <input 
                  v-model="inputUrl" 
                  type="text" 
                  placeholder="粘贴网易云/QQ音乐单曲或歌单分享链接..." 
                  class="flex-1 bg-slate-950 border border-slate-700/50 rounded p-3 text-slate-100 text-sm focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500"
                ...
                >
                  <span v-if="isParsing" class="animate-spin text-lg block w-4 h-4 rounded-full border-2 border-t-white border-white/20"></span>
                  剥离解析
                </button>
              </div>
            </div>
            ...
            <div class="flex flex-col gap-2">
              ...
              <select v-model="selectedLevel" :disabled="isParsing || isDownloading" class="bg-slate-950 border border-slate-700/50 rounded p-3 text-slate-300 text-sm focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 w-full">
                <option value="standard">标准音质 (Standard - 128k)</option>
                <option value="exhigh">极高音质 (Exhigh - 320k)</option>
                <option value="lossless">无损音质 (Lossless - FLAC)</option>
                <option value="hires">Hi-Res 母带 (Hi-Res)</option>
              </select>
            </div>
            
            <!-- Cookie Input Area (Collapsible) -->
            <div v-show="showCookieConfig" class="flex flex-col gap-2 bg-slate-950/50 p-3 rounded border border-slate-700/50">
              <label class="text-xs font-semibold text-slate-400">
                权限凭证 (Cookie) 
                <span class="text-[10px] text-slate-500 font-normal ml-2">解锁无损音质必填，已本地加密保存</span>
              </label>
              <textarea 
                v-model="cookieData" 
                placeholder="在此粘贴包含 MUSIC_U 一整大段的 Cookie 文本..." 
                class="w-full h-16 bg-slate-900 border border-slate-700 rounded p-2 text-slate-300 text-xs focus:outline-none focus:border-indigo-500 font-mono resize-none"
                @input="saveCookie"
              ></textarea>
            </div>
          </div>
          ...
          <div v-if="parsedData" class="flex flex-col gap-4 border border-slate-800 bg-slate-800/20 p-4 rounded-xl">
             <div v-if="parsedData.type === 'song'" class="flex items-center gap-4">
                ...
                <div class="flex flex-col items-end">
                  <span v-if="parsedData.data[0].downloadable" class="px-2 py-0.5 rounded text-[10px] uppercase font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">直链穿透成功</span>
                  <span v-else class="px-2 py-0.5 rounded text-[10px] uppercase font-bold bg-rose-500/10 text-rose-400 border border-rose-500/20">被封锁 / VIP专属</span>
                </div>
             </div>
             ...
             <div v-else-if="parsedData.type === 'playlist'" class="flex items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                   ...
                   <p class="text-slate-400 text-sm truncate">包含 {{ parsedData.trackIds?.length || 0 }} 首锁定中继节点</p>
                </div>
                <div class="px-2 py-0.5 rounded text-[10px] uppercase font-bold bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">歌单集合</div>
             </div>
             ...
             <div class="flex border-t border-slate-700/50 pt-4 mt-2 justify-end gap-3">
               ...
               <button 
                  ...
                  class="px-5 py-2.5 bg-rose-600 hover:bg-rose-500 text-white rounded font-bold text-sm transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
                  下发收纳指令
               </button>
             </div>
          ...
          <!-- 下载进度与日志 -->
          <div v-if="logs.length > 0" class="flex-1 border border-slate-800 bg-slate-950 rounded-xl overflow-hidden flex flex-col min-h-[200px]">
             <div class="bg-slate-800 px-3 py-2 text-xs text-slate-400 font-mono tracking-wider flex justify-between">
                <span>[中枢终端] - EXECUTION LOG</span>
             </div>
    ...
    try {
        ...
        if (res.data.success) {
            ...
            if (res.data.type === 'playlist') {
                addLog(`INFO: 解析到歌单队列，共计 ${res.data.trackIds?.length} 首。`);
            } else {
                addLog(`INFO: 锁定单曲，状态：${res.data.data[0].downloadable ? '允许访问' : '遭封禁'}`);
            }
        }
    } catch (e: any) {
        addLog(`ERR: 解析链路崩溃 - ${e.response?.data?.error || e.message}`);
    } finally {
...
const downloadSingle = async (id: number | string): Promise<boolean> => {
    try {
        ...
        addLog(`ERR: 下载拒绝服务 - ${e.response?.data?.error || e.message}`);
        return false;
    }
}
...
const startDownload = async () => {
    ...
    try {
        if (parsedData.value.type === 'song') {
            ...
            addLog(`WARN: 开始集群式列阵收割，总计 ${ids.length} 首。`);
            ...
            for (let i = 0; i < ids.length; i++) {
                ...
                addLog(`[${i+1}/${ids.length}] 预热提取目标 ID:${ids[i]}`);
            ...
            if (!isCancelled.value) {
                 addLog(`🎉 集群收割全部执行完毕！成果统计：成功落地 [${successCount}] 首 | 绝版拦截废弃 [${failedCount}] 首。`);
            }
        }
