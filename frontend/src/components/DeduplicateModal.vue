    ...
    <div class="relative w-full max-w-4xl bg-slate-800 rounded-xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden border border-slate-700">
      ...
      <div class="flex-1 overflow-y-auto p-6 bg-slate-800/50">
        <div v-if="loading" class="flex flex-col items-center justify-center py-20 text-indigo-400 gap-4">
           ...
           <span>{{ t('ui.dedupe.scanning') }}</span>
        </div>
        ...
        <div v-else-if="duplicates.length === 0" class="flex flex-col items-center justify-center py-20 text-emerald-400 gap-3">
          ...
          <p class="text-lg">{{ t('ui.dedupe.no_duplicates') }}</p>
          <span class="text-xs text-slate-500">{{ t('ui.dedupe.compare_hint') }}</span>
        </div>

        <div v-else class="space-y-6">
          <div class="bg-indigo-50 border border-slate-700/30 text-slate-400 p-4 rounded text-sm leading-relaxed shadow-inner">
            <span v-html="t('ui.dedupe.desc', { count: duplicates.length })"></span>
          </div>
          ...
          <div v-for="(group, idx) in duplicates" :key="idx" class="bg-slate-900 p-4 rounded-xl border border-slate-700 shadow-inner">
            <h4 class="font-medium text-slate-100 flex justify-between items-center mb-4 border-b border-slate-700/50 pb-2">
              ...
              <span class="text-xs bg-slate-800 text-slate-400 px-2 py-0.5 rounded-full border border-slate-600">{{ t('ui.dedupe.group_count', { count: group.files.length }) }}</span>
            </h4>
               ...
               <label v-for="(file, findex) in group.files" :key="file.id" 
                 ...
                 <div class="flex-1 min-w-0">
                    <p class="text-sm font-mono text-slate-300 truncate" :title="file.filepath">
                      <span v-if="findex === 0 && !file.deleted" class="px-1.5 py-0.5 rounded bg-emerald-900/30 text-emerald-400 text-[10px] border border-emerald-700/40 mr-2 uppercase tracking-wider">{{ t('ui.dedupe.best_ver') }}</span>
                      {{ file.filename }}
                    </p>
                    <p class="text-xs text-slate-500 mt-1 flex gap-4">
                      <span>{{ t('ui.dedupe.format') }} <strong class="text-slate-400 uppercase">{{ file.extension.replace('.','') }}</strong></span>
                      <span>{{ t('ui.dedupe.size') }} <strong class="text-slate-400">{{ formatBytes(file.size) }}</strong></span>
                      <span v-if="file.status === 1" class="text-emerald-500">{{ t('ui.dedupe.scraped') }}</span>
                      <span v-else class="text-amber-500/70">{{ t('ui.dedupe.not_scraped') }}</span>
                    </p>
                 </div>
                 <span v-if="file.deleted" class="text-xs bg-rose-900/50 text-rose-400 px-2 py-1 rounded">{{ t('ui.dedupe.deleted_tag') }}</span>
               </label>
            </div>
          </div>
        </div>
      ...
      <div class="p-5 border-t border-slate-700 bg-slate-900/80 flex justify-between items-center rounded-b-xl">
        <span class="text-sm text-slate-400">{{ t('ui.dedupe.selected_count', { count: selectedIds.length }) }}</span>
        <div class="flex gap-3">
          <button @click="$emit('close')" class="px-5 py-2.5 rounded bg-slate-700 hover:bg-slate-600 text-white transition text-sm font-medium">{{ t('ui.dedupe.close_btn') }}</button>
          <button @click="deleteSelected" :disabled="selectedIds.length === 0 || deleting" class="px-5 py-2.5 rounded bg-rose-600 hover:bg-rose-500 text-white flex items-center gap-2 transition text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-rose-900/20">
            ...
            {{ deleting ? t('ui.dedupe.deleting') : t('ui.dedupe.delete_btn') }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-if="confirmDialog.isOpen" class="fixed inset-0 z-[70] flex items-center justify-center p-4 backdrop-blur-sm bg-slate-900/60 transition-all">
        <div class="bg-slate-800 border border-slate-700 rounded-lg shadow-2xl p-6 max-w-sm w-full">
            <h3 class="text-xl font-bold text-white mb-2 flex items-center gap-2">
                ...
                {{ t('ui.dedupe.warning_title') }}
            </h3>
            <p class="text-slate-300 text-sm mb-6">{{ confirmDialog.message }}</p>
            <div class="flex justify-end gap-3 flex-wrap">
                <button @click="confirmDialog.isOpen = false" class="px-4 py-2 rounded bg-slate-700 hover:bg-slate-600 transition text-white text-sm font-medium">{{ t('ui.dedupe.close_btn') }}</button>
...
const deleteSelected = async () => {
    ...
    confirmDialog.value = {
        isOpen: true,
        message: t('ui.dedupe.warning_msg', { count: selectedIds.value.length })
    };
};
...
const proceedDeletion = async () => {
    ...
    try {
        ...
        if (result.success) {
            ...
            selectedIds.value = [];
            showToast(t('ui.dedupe.delete_success'), 'success');
            emit('deleted');
        } else {
            showToast(t('ui.dedupe.delete_fail', { error: result.error }), 'error');
        }
    } catch (e) {
        showToast(t('ui.dedupe.network_error'), 'error');
    } finally {
